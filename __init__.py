...
# - fan_speed_level kann jetzt direkt geändert werden
...
class Pluggit(SmartPlugin):
    ...
    _modbusRegisterDic = {
        'prmCurrentIPAddress': 29,              # 40029: current IP-Adress
        # 'prmCurrentIPAddress_low': 29,        # 40029: current IP-Adress (low)
        # 'prmCurrentIPAddress_high': 30,       # 40030: current IP-Adress (high)
        # 'prmCurrentIPMask_low': 33,           # 40033: current IP-Mask (low)
        # 'prmCurrentIPMask_high': 34,          # 40034: current IP-Mask (high)
        # 'prmCurrentIPGateway_low': 37,        # 40037: current IP-Gateway (low)
        # 'prmCurrentIPGateway_high': 38,       # 40038: current IP-Gateway (high)
        # 'prmMACAddrHigh_low': 41,             # 40041: current MAC-Adress high (low)
        # 'prmMACAddrHigh_high': 42,            # 40042: current MAC-Adress high (high)
        # 'prmMACAddrLow_low': 43,              # 40043: current MAC-Adress low (low)
        # 'prmMACAddrLow_high': 44,             # 40044: current MAC-Adress low (high)
        # 'prmDateTime': 109,                   # 40109: Current Date/Time in Unix time (amount of seconds from 1.1.1970)
        'prmHALTaho1': 101,                     # 40101: fan1 rpm
        'prmHALTaho2': 103,                     # 40103: fan2 rpm
        'prmRamIdxT1': 133,                     # 40133: T1, °C
        'prmRamIdxT2': 135,                     # 40135: T2, °C
        'prmRamIdxT3': 137,                     # 40137: T3, °C
        'prmRamIdxT4': 139,                     # 40139: T4, °C
        # 'prmRamIdxT5': 141,                   # 40141: T5, °C
        'prmRamIdxUnitMode': 168,               # 40169: Active Unit mode> 0x0004 Manual Mode; 0x0008 WeekProgram
        'prmRamIdxBypassActualState': 199,      # 40199: Bypass state> Closed 0x0000; In process 0x0001; Closing 0x0020; Opening 0x0040; Opened 0x00FF
        ...
        'prmBypassTmin': 445,                   # 40445: Minimum temperature of Bypass openning (°C), if T1 < Tmin then bypass should be closed
        'prmBypassTmax': 447,                   # 40447: Maximum temperature of Bypass openning (°C), if T1 > Tmax or Tmax is 0 then bypass should be closed
        # 'prmWorkTime': 625                    # 40625: Work time of system, in hour (UNIX)
        'prmNumOfWeekProgram': 467,             # 40467: Number of the Active Week Program (for Week Program mode)
        ...
        try:
            ...
            for pluggit_key in self._myTempReadDict:
                ...
                if values == self._modbusRegisterDic['prmCurrentIPAddress']:
                    ip1 = self._Pluggit.read_holding_registers(values, 2, unit=22)
                    decoderip1 = BinaryPayloadDecoder.fromRegisters(ip1.registers, byteorder=Endian.Big, wordorder=Endian.Big)
                    self.logger.debug("Pluggit: current IP adress: {0}".format(decoderip1))
                ...
                if values == self._modbusRegisterDic['prmHALTaho1']:
                    ...
                    decoderf1 = BinaryPayloadDecoder.fromRegisters(f1.registers, byteorder=Endian.Big, wordorder=Endian.Big)
                ...
                if values == self._modbusRegisterDic['prmHALTaho2']:
                    ...
                    decoderf2 = BinaryPayloadDecoder.fromRegisters(f2.registers, byteorder=Endian.Big, wordorder=Endian.Big)
                ...
                if values == self._modbusRegisterDic['prmRamIdxT1']:
                    ...
                    decodert1 = BinaryPayloadDecoder.fromRegisters(t1.registers, byteorder=Endian.Big, wordorder=Endian.Big)
                ...
                if values == self._modbusRegisterDic['prmRamIdxT2']:
                    ...
                    decodert2 = BinaryPayloadDecoder.fromRegisters(t2.registers, byteorder=Endian.Big, wordorder=Endian.Big)
                ...
                if values == self._modbusRegisterDic['prmRamIdxT3']:
                    ...
                    decodert3 = BinaryPayloadDecoder.fromRegisters(t3.registers, byteorder=Endian.Big, wordorder=Endian.Big)
                ...
                if values == self._modbusRegisterDic['prmRamIdxT4']:
                    ...
                    decodert4 = BinaryPayloadDecoder.fromRegisters(t4.registers, byteorder=Endian.Big, wordorder=Endian.Big)
                ...
                if values == self._modbusRegisterDic['prmBypassTmin']:
                    ...
                    decodert5 = BinaryPayloadDecoder.fromRegisters(t5.registers, byteorder=Endian.Big, wordorder=Endian.Big)
                ...
                if values == self._modbusRegisterDic['prmBypassTmax']:
                    ...
                    decodert6 = BinaryPayloadDecoder.fromRegisters(t6.registers, byteorder=Endian.Big, wordorder=Endian.Big)
